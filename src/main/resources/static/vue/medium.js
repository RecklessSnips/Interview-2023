...
const app = Vue.createApp({
  ...
  methods: {
    ...
    getMistake() {
      ...
      // if user made no mistakes on previous page, initialize a new array
      this.mistakes = storedMistakes ? JSON.parse(storedMistakes) : [];
    },
    // get hhow many mistakes has been made
    getMistakeID() {
    ...
    getTF() {
      return fetch(
        this.questions +
          "&category=" +
          this.gameType +
          "&difficulty=medium&type=boolean"
      )
        .then((response) => {
        ...
        .then((data) => {
          ...
          if (this.response.response_code !== 1) {
            ...
            return fetch(
              "https://opentdb.com/api.php?amount=1&difficulty=medium&type=boolean"
            )
              .then((response) => {
                return response.json();
              })
